...
class Node:
    ...
    def deliver(self, msg_type, value):
        ...
        if msg_type == "heartbeat":
            ...
            candidate_id = value
            #if node recieves another candidacy message before the waiting time ends then cancel candidacy
            if self.state == 'candidate' and now < self.wait_until:
            	self.state = 'follower'
            	self.wait_until = None
            	self.candidate = False
            	self.election_timeout = None
            ...
            if self.voted is None:
                ...
                #we should add random voting logic here(?)
    ...
    def election(self):
        ...
        if self.working and time.time() >= wait_until:
            ...
            if time.time() >= self.election_timeout and len(self.num_of_received_votes) > len(nodes) // 2:
                self.state = 'leader'
                self.election_timeout = None
